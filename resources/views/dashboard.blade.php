...
@section('content')
    ...
    {{-- HERO HEADER --}}
    <div class="mb-6 sm:mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 min-w-0">
        <div class="min-w-0">
            <div class="flex items-center gap-2 mb-1">
                <span class="live-dot shrink-0"></span>
                <p class="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-emas-dim truncate">Portal Pemantauan &amp; Analitik Inflasi</p>
            </div>
            <h1 class="font-display text-2xl sm:text-3xl font-extrabold tracking-tight text-ink break-words">Ringkasan Inflasi Kota Padang</h1>
            <p class="text-ink-soft text-xs sm:text-sm mt-1">Pemantauan angka inflasi, sumbangan kelompok pengeluaran, pergerakan harga pangan, dan ketahanan stok.</p>
        </div>

        {{-- Filter Utama & Kotak Cari Data --}}
        <form method="GET" action="{{ route('dashboard') }}" class="flex flex-wrap items-center gap-2 shrink-0">
            ...
            <input type="hidden" name="tahun_stok" value="{{ $tahunStok }}">
            <input type="hidden" name="bulan_stok" value="{{ $bulanStok }}">

            {{-- Search Box --}}
            <div class="relative min-w-[140px] sm:min-w-[180px]">
                <svg viewBox="0 0 20 20" class="h-3.5 w-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-ink-soft/70 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <input type="text" name="q" value="{{ $searchQuery ?? '' }}" placeholder="Cari data..."
                    class="w-full rounded-xl border border-garis bg-white/90 pl-8 pr-3 py-2 text-xs font-semibold text-ink placeholder:text-ink-soft/60 shadow-2xs hover:border-emas focus:border-emas focus:outline-none transition-all">
            </div>

            {{-- Tahun Filter --}}
            <div class="relative">
                <select name="tahun_inflasi" onchange="this.form.submit()"
                    class="appearance-none rounded-xl border border-garis bg-white/90 pl-3 pr-8 py-2 text-xs font-semibold text-ink shadow-2xs hover:border-emas/50 focus:border-emas focus:outline-none transition-all cursor-pointer">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-5 mb-6 sm:mb-8 min-w-0">
        {{-- Card 1: YoY Padang --}}
        <div class="kartu kartu-hover p-4 sm:p-5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6 mb-6 sm:mb-8 min-w-0">
        ...
        <div class="lg:col-span-2 kartu p-4 sm:p-6 overflow-hidden">
            ...
            <div class="h-64 sm:h-72 w-full">
                <canvas id="chartTrenYoY"></canvas>
            </div>
        </div>

        {{-- GRAFIK 2: Andil Inflasi per Kelompok (Filter Mandiri Tahun & Bulan Andil) --}}
        <div class="kartu p-4 sm:p-6 overflow-hidden flex flex-col justify-between">
            <div>
                ...
                <form method="GET" action="{{ route('dashboard') }}" class="flex flex-wrap items-center gap-2 mb-4 bg-sand-dark/20 p-2 rounded-xl border border-garis/50">
            ...
            <div class="h-56 w-full">
                <canvas id="chartAndilYoY"></canvas>
            </div>
        </div>
    </div>

    {{-- BAGIAN 3: GRAFIK HARGA PANGAN STRATEGIS VS HET (LINE CHART) --}}
    <div class="kartu p-4 sm:p-6 overflow-hidden mb-6 sm:mb-8 min-w-0">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 border-b border-garis/40 pb-3">
            <div>
                <div class="flex items-center gap-2">
        ...
        <div class="h-64 sm:h-72 w-full">
            <canvas id="chartPanganMingguan"></canvas>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 min-w-0">
        {{-- Top Andil --}}
        <div class="kartu p-4 sm:p-6 overflow-hidden">
            <div class="flex items-center justify-between gap-2 mb-3">
                ...
                <a href="{{ route('andil.index') }}" class="text-xs font-bold text-marun hover:underline">Lihat Detail &rarr;</a>
            </div>
        ...
        <div class="kartu p-4 sm:p-6 overflow-hidden">
            <div class="flex items-center justify-between gap-2 mb-3">
                ...
                <a href="{{ route('pangan.index') }}" class="text-xs font-bold text-marun hover:underline">Lihat Detail &rarr;</a>
            </div>
...
<script>
    ...
    const ctxBarYoY = document.getElementById('chartTrenYoY').getContext('2d');

    // Gradasi 3 Warna Padang (Soft Crimson Maroon)
    const gradPadang = ctxBarYoY.createLinearGradient(0, 0, 0, 280);
    ...
    const gradSumbar = ctxBarYoY.createLinearGradient(0, 0, 0, 280);
    ...
    const gradNasional = ctxBarYoY.createLinearGradient(0, 0, 0, 280);
    ...
    new Chart(document.getElementById('chartAndilYoY'), {
        type: 'bar',
        data: {
    ...
    new Chart(document.getElementById('chartPanganMingguan'), {
        type: 'line',
        data: {
