...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $andilQuery = AndilInflasi::where('tahun', $tahunAndil)
            ->where('urutan_bulan', $bulanAndil)
            ->where('kelompok', '!=', 'Umum (Headline)');

        if ($searchQuery !== '') {
            $andilQuery->where('kelompok', 'like', "%{$searchQuery}%");
        }

        $andilSemua = $andilQuery->orderByDesc('andil_yoy')->get()->unique('kelompok')->values();
        $andilTertinggi = $andilSemua->take(4)->values();
        ...
        // Section 3: Harga Pangan Per Komoditas & Tahun
        ...
        $panganQuery = HargaPangan::where('tahun', $tahunPangan)
            ->where('urutan_bulan', $bulanTerakhirPangan)
            ->where('minggu', $mingguTerakhir);

        if ($searchQuery !== '') {
            $panganQuery->where('komoditas', 'like', "%{$searchQuery}%");
        }

        $panganSemua = $panganQuery->get()->unique('komoditas')->values();
        ...
        $stokPanganQuery = \App\Models\StokPangan::where('tahun', $tahunStok)
            ->where('urutan_bulan', $bulanStok);

        if ($searchQuery !== '') {
            $stokPanganQuery->where('komoditas', 'like', "%{$searchQuery}%");
        }

        $stokPanganList = $stokPanganQuery->orderBy('komoditas')->get();

        if ($stokPanganList->isEmpty() && $searchQuery === '') {
